...
The first PC explains the most variance, however this is still just 41.2% of the overall variance; the second PC accounts for 14.9%, and the rest of the variance is split over many subsequent PCs (Figure \@ref(fig:screeplots)).The first two PCs account for 56.18% of the variance. PC3 and PC4 account for 8.6 and 6.6% respectively, in total explaining 15.2%, so these were also kept for analysis. The first 4 PCs account for 70.56% of the variance. Likewise, the eigenvalues for PC1 and PC2 are 7.8 and 2.8, which equates to 41.2% and 14.9% of the sum total of eigenvalues, which is 19.
...
Looking at the variable loadings, are there 9 variables which are strongly negatively correlated with PC1, with maxwid and weight being the strongest at 0.92 and 0.89 respectively (Table \@ref(tab:variable-loadings)). 
...
Maxthick, blawid, maxleng, blathick, longside, grinside and blaleng are the other 7 correlated variables. For PC2, its strongest correlations are with polthick and polwid at -0.77 and -0.71 respectively, followed by lonpoled, longside and poleng. PC3 has no particularly strong correlations however it is moderately negatively correlated with poleng and lengmaxt at -0.52 and -0.42, and positively with bladang and blathick at 0.44 and 0.42. Finally, PC4’s correlations are weaker still, with its strongest being poledwid at -0.51, gripoled at 0.51 and lefedwid at 0.44.
...
Examining PCs other than 1 and 2 can reveal outliers which were not visible on the first two PCs, such as 81004 which has the largest poleng measurement at 52mm, and 60126 which has an abnormally small poleng at 1.5mm but a bladang of 180 degrees. The arrangement of the variables suggests that axes with larger bladangs and blawids are to the right of PC3. As the variable loadings are relatively weak for these PCs, in addition to how little variance they account for, it is difficult to discern the relations between axes on either side of PCs 3 or 4 (Shennan 1980, p267).
...
In light of these results, k-means analyses were performed with clusters set to 2 and 3 (Figure \@ref(fig:k-means)). A 4-cluster solution was also trialed to see if any of the groups would retain their membership if more clusters were imposed. The axes visible on the far right of the plot in cluster 1 of the 2-cluster arrangement are the same as those which were on the far right of PC1 in the PCA, which have the largest measurements for maxwid, weight, maxleng, and other variables which correlate strongly with PC1. These appear to be lumped in with smaller axes, such as 60049, weighing 310g with a maxleng of 96mm. Cluster 2 contains the most individuals, and has the same axes on its far left which were on the far left of PC1.
...
k3 <- fviz_cluster(kclust3, data = scaled_neo_axes, labelsize=6) +
  my_theme
k4 <- fviz_cluster(kclust4, data = scaled_neo_axes, labelsize=6) +
  my_theme
...
Moving onto the 3 cluster plot, the axes on the extremities of the plot are still in the same places, however many of the axes which were on the borders between clusters 1 and 2 previously have now been relocated to cluster 1. Most of the axes tend to fall into clusters 1 and 2, and cluster 3, which still contains the largest axes, has decreased in size. Here we can see better separation between the cluster containing the longer/wider axes and the other clusters, however there is overlap between clusters 1 and 2, which also are similar sizes. Clusters 1 and 2 are partitioned through where most of the axes fall, almost appearing to overlap, which suggests poor partitioning or overfitting (Rhys 2019, p403).

The 4-cluster plot has reassuringly made very little changes to the previous cluster 1 membership from the 3-cluster analysis, which has now become cluster 4, however some of the bordering axes were moved into cluster 3. There overlapping is now worse, and involves the three other clusters, which appear to be splitting closely clustered individuals into separate groups arbitrarily. Because the 4-cluster k-means analysis confirmed that cluster 1 in the 3-cluster solution does not change with more clusters imposed, the solution with 3 groups was considered the best of these options.

### Within-cluster densities

It is clear that cluster 1 mostly contains the axes with the largest values for blaleng, blathick, blawid, lengmaxt, longside, maxleng, maxthick, maxwid and weight (Figure \@ref(fig:cluster-densities)). Interestingly, cluster 1 also has the widest distribution for weight, polwid, maxthick, lengmaxw and blaleng. Cluster 1 tends to have a mode close to cluster 2’s on many variables, but cluster 2 generally is dominated by axes with the smallest measurements for most variables. Cluster 1 also seems to have similar distributions to cluster 3 of polwid, polthick, lonpoled and perhaps bladang. The bimodal distributions of grinside and gripoled are still evident here and it is clear which clusters cause these. Clusters 1 and 2 cause the mode at lower values for grinside and cluster 3 causes the mode at higher values, whilst cluster 2 and clusters 1 and 3 combined create the two modes for gripoled. Bladang, poledwid and poleng do not show much difference in distributions between the clusters.
...
The clusters are very clearly visible on PC1 and PC2, but are amalgamated on PC3 and PC4 (\@ref(fig:pca-cluster)). Clusters 1 and 2 overlap, appearing to split an already existing cluster in two, and cluster 3 is shown trailing off to the right, containing the axes with larger measurements for the variables correlated with PC1. Cluster 2 contains the majority of axes. Overall the arrangement of the clusters suggests that membership depends on the size of the measurements an axe has for the variables correlated with PC1. Interestingly, cluster 1 contains the three extreme outliers on PC3 and PC4.
