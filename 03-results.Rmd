...
The biplot shows the variables which were most correlated with PC1 correlating with one another, and likewise for those which correlate the most with PC2 (Figure \@ref(fig:no-na-pca)). Interestingly, PC2 is characterised by all of the poll measurements, whilst PC1 relates to measurements which define the overall size of an axe. This is reassuringly similar to the PCA results from the larger dataset with 9 variables, as poledwid and lonpoled were correlated with PC2 and the overall size features correlated with PC1.

```{r no-na-pca, out.width="90%", fig.cap="PCA with 19 variables measured for 369 axes."}
...
Moving onto the 3 cluster plot, the axes on the extremities of the plot have remained the same, however many of the axes which were on the borders between clusters 1 and 2 previously have now been relocated to cluster 1. Most of the axes tend to fall into clusters 1 and 2, which are similar sizes. This solution has removed some of the smaller axes from the cluster containing the largest axes, which is an improvement, however all clusters are partitioned through where most of the axes fall, almost appearing to overlap, which suggests poor partitioning or overfitting [@rhys2019 403]. Cluster 2 contains the axes with the largest measurements for bladang, gripoled and lonpoled.

The 5-cluster plot has reassuringly made negligible changes to the previous cluster 1 membership from the 3-cluster analysis, which has now become cluster 4. Cluster 2 also appears to have stayed more or less the same as cluster 3 from the 3-cluster plot, but overlaps considerably with the other three clusters, which were previously one middle cluster. Because this potential solution has made little changes to clusters 1 and 3 from the 3-cluster solution, plus clusters 2, 3 and 4 appear to be splitting one cluster, the 3-cluster solution was selected as the best of these options.
...
In the panel plot with the clusters numbered and colour-coded, the widest axes are clearly in cluster 4, and the most narrow in cluster 1 (Figure \@ref(fig:panel-clusters)). The two similar clusters, 2 and 3, which were in the centre of the k-means plot, do appear slightly different here, with the axes in 2 mostly being wider than those in 3. Axes in cluster 2 also appear more trapezoidal than cluster 3. Overall it seems that the aspect ratio of the outlines is the deciding factor in the groupings here, however the two wider groups tend to have more axes with wider blades than polls compared to the two narrower groups. Cluster 4 also contains all of the most square axes in the dataset, further suggesting that width of shapes is the cause for this partitioning.
...
```{r maps, fig.align="center", fig.cap="Find places of axes in each cluster plotted onto a map of Britain, with measurement clusters above and outline clusters below."}
