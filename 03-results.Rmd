...
#neo_axes_PCA_nona <- prcomp(scaled_neo_axes)
...
As the dotplot showed that there were 388 axes which had measurements for 19 variables from maxwid upwards, and since those variables include some useful descriptors of shape, PCA was attempted again with these 19 variables but a reduced dataset of 390. For brevity, only PCs 1 and 2 will be analysed here (Figure @/ref(fig:no-na-variable-loadings). The variable loadings show that 9 variables are correlated with PC1, with weight and maxwid being the strongest at 0.92 and 0.89 respectively; these are followed by maxthick, blawid, maxleng, blathick, longside, grinside and lengmaxt. For PC2, the strongest correlations are with polwid and polthick at -0.71 and -0.77, followed by moderate correlations with lonpoled, poleng, maxleng and poledwid.

```{r no-na-variable-loadings}
pca_no_na <- drop_na(neo_axes_wide19) %>%
  prcomp(center = TRUE, scale = TRUE)

map_dfc(1:4, ~pca_no_na$rotation[, .] * abs(pca_no_na$sdev)[.]) %>%
  mutate(across(everything(), round, digits = 2)) %>%
  set_rownames(rownames(pca_no_na$rotation)) %>%
  set_colnames(c("PC1", "PC2", "PC3", "PC4")) %>%
  arrange(desc(abs(PC1))) %>%
  kable(row.names=TRUE, 
...
```{r no-na-pca, fig.cap="PCA with 19 variables measured for 388 axes."}

fviz_pca_biplot(pca_no_na, title=element_blank(), col.var="black", 
                col.ind="skyblue1") +   coord_fixed()
```
...
scaled_neo_axes <- neo_axes_wide_imputed$completeObs %>%
  # drop_na() %>%
  scale()
...
In light of these results, k-means analyses were performed with clusters set to 2, 3 and 5 (Figure \@ref(fig:k-means)). The axes visible on the far right of the plot in cluster 1 of the 2-cluster arrangement are the same as those which were on the far right of PC1 in the PCA, which have the largest measurements for maxwid, weight, maxleng, and other variables which correlate strongly with PC1. These appear to be lumped in with smaller axes, such as 81005, weighing 285g, with a longside of 95mm. Cluster 2 contains the most individuals, and has the same axes on its far left which were on the far left of PC1.

```{r k-means, echo=FALSE, message=FALSE, fig.cap="K-means analyses with 2, 3 and 5 clusters."}
...
panel(outlines,
      ...
      names = as.factor(kmeans4$cluster),
      cex.names=0.5) 
#dev.off()
...
mp_kmeans <- enframe(kclust2$cluster, name = "axe_id", value = "mp_cluster")

mm_kmeans <- enframe(kmeans4$cluster, name = "stem", value = "mm_cluster") %>%
  mutate(axe_id = str_extract(stem, "[0-9]{2,}[ab]*"))
...
mp_ef <- MSHAPES(ef, fac = as.factor(mp_clusters))
Out(mp_ef$shp) %>% panel(names = TRUE)
...
neo_axes_coords_kclust <-
  neo_axes %>%
  filter(!is.na(lat)) %>%
  left_join(kclust3_lookup, by = "id") %>%
  filter(!is.na(cluster))

britain <- c(left = -8.1, bottom = 49.9, right = 2, top = 59)
britain_map <- get_stamenmap(britain, zoom = 7, maptype = "terrain-background")

ggmap(britain_map, extent = 'device') +
# ggplot(neo_axes_coords_clustered, aes(x = lon, y = lat, alpha = 0.5)) +
  geom_point(aes(colour = cluster, x = lon, y = lat), alpha = 0.8,
             neo_axes_coords_kclust, size = 1.5) +
  # coord_map() +
  # theme_stripped +
  # theme_empty +
  theme(legend.position = "right")
