...
```{r density-plots, message=FALSE, echo=FALSE, fig.width=6, fig.cap="Density plots. Red line: mean; blue line: median."}
...
The WSS plot displayed an ‘elbow’ or sudden change at 2 and 3, whilst the average silhouette overwhelmingly indicated 2 as the optimal number of clusters (Figure \@ref(fig:wss-silhouette)). Both plots also show a slight suggestion that there may be 9 clusters. In light of these results, k-means analyses were performed with clusters set to 2 and 3 (Figure \@ref(fig:k-means)). 
...
p1 + p2
```
The axes visible on the far right of the plot in cluster 1 of the 2-cluster arrangement are the same as those which were on the far right of PC1 in the PCA, which have the largest measurements for maxwid, weight, maxleng, and other variables which correlate strongly with PC1. These appear to be lumped in with smaller axes, such as 60049, weighing 310g with a maxleng of 96. Cluster 2 contains the most individuals, and has the same axes on its far left which were on the far left of PC1.
...
It is clear that cluster 1 mostly contains the axes with the largest values for blaleng, blathick, blawid, lengmaxt, longside, maxleng, maxthick, maxwid and weight (Figure \@ref(fig:cluster-densities)). 
...
Clusters 1 and 2 tend to overlap on most variables, but cluster 2 generally is dominated by axes with the smallest measurements for many of the variables relating to size.
...
sc1 <- fviz_screeplot(neo_axes_PCA, addlabels = TRUE, choice = "eigenvalue", geom="bar", barfill="thistle") +
  my_theme
sc2 <- fviz_screeplot(neo_axes_PCA, addlabels = TRUE, choice = "variance", geom="bar", barfill="lavender") +
  my_theme
...
The biplot for PC1 and PC2 shows that most of the variables correlate with one another, with the exception of polwid, polthick, poleng, bladang, and lonpoled, which correlate with one another but less so with the rest (Figure \@ref(fig:pc1-pc2)). Poledwid correlates the least with any other variable. Interestingly, grinside and maxleng are perfectly correlated, as are blaleng and blathick, and bladang and lonpoled. Lengmaxt and lengmaxw are almost perfectly correlated. The arrangement of the individuals on PC1 and PC2 shows long, heavy, wide axes with thick blades on the far left side, and the shortest, lightest, most narrow axes with the thinnest blades on the far right. For example, 70002 on the far right weighs 1565g, has a maxleng of 226mm, a maxwid of 59mm and blathick of 36mm. Similarly 60001 weighs 1860g with a maxleng of 299mm, blathick of 29mm and maxwid 79mm. Meanwhile on the far left examples include 10003 which weighs just 60g with a maxleng of 86 and 70006 weighing 56g with a maxleng of 81. To the left of the origin we see the majority of the axes. There are some outliers toward the bottom of PC2 which score highly on the poll measurements such as 60049 which has the largest polthick in the dataset at 31mm, with a 58mm polwid and 18mm poleng.
...
fviz_pca_biplot(neo_axes_PCA) +   coord_fixed()
...
For PC3 and PC4, the biplot shows many variables negatively correlating with one another, with strong positive correlations between polwid and lonpoled, and longside and maxleng (Figure \@ref(fig:pc3-pc4)). Examining PCs other than 1 and 2 can reveal outliers which were not visible on the first two PCs, such as 81004 which has the largest poleng measurement at 52mm, and 60126 which has a bladang of 180 degrees. It appears as though axes with larger bladangs and blawids are to the right of PC3. As the variable loadings are relatively weak for these PCs, in addition to how little variance they explain, it is difficult to discern the relations between axes on either side of PCs 3 or 4 (Shennan 1980, p267).
...
fviz_pca_biplot(neo_axes_PCA, axes=c(3,4), col.ind="skyblue", col.var="black") +   coord_fixed()
...
```{r pca-cluster, fig.height=10, fig.width=10, fig.cap = "Clusters colour-coded onto plots of the two pairs of axes."}
...
error_fills <- read.csv("data/error-fills.csv", header=F)$V1 %>% as.character()

no_errors <- axe_lookup[!axe_lookup$axe_id %in% error_fills, ]

correct_files <- no_errors$file

outlines <- import_jpg(correct_files) %>% Out()
...
panel(outlines, palette=col_spring, 
      fac=as.factor(kmeans3$cluster), names=as.factor(no_errors$axe_id), 
