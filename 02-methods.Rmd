...
This dataframe also included the area and aspect ratio of each view, which was used to determine which view had been cropped, and this was used in the filename (code snippet). The dataframe is exported to csv for each axe, which the Makefile looks in for instructions. 
...
The `Makefile` crops the views using the `convert` function from the `imagemagick` program and a formula which is replaced with the coordinates for the bounding boxes. Finally, the cropped axe views are filled in by `scripts/fill_view.py` in three steps. The outline is first filled by finding the contours again, but this time filling them when drawing over the image (code snippet; Figure \@ref(fig:filled-conts)).
...
cv2.drawContours(outlines4, large_cont, -1, (0,0,0), \
thickness=cv2.FILLED)
```
